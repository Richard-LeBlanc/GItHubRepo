...
function Coffee(size, isDecaf, qtyCream, qtySugar){
    ...
    this.description = function (){
        output.textContent = `This ${this.size} ${this.decaf} coffee has ${this.qtyCream >=  1 ? qtyCream : "no"} cream and ${this.qtySugar >= 1 ? this.qtySugar : "no"} sugar.`;
    };
    this.setDecaf = function(){
        this.decaf = (isDecaf === true) ? "decaffeinated" : "caffeinated";
    }
}
/* STEP 2: Instatiate a coffee based on the above constructor function */
let myCoffee = new Coffee("medium", false, 1);
...
let robertsCoffee = Object.create(myCoffee);
robertsCoffee.size = "large";
robertsCoffee.isDecaf = true;
robertsCoffee.qtyCream = 2;
robertsCoffee.qtySugar = 2;
...
let meganCoffee = new robertsCoffee.constructor("small", true, 3, 1);
